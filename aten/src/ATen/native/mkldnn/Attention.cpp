...
thread_local std::unordered_map<int32_t, dnnl::graph::partition>
    partition_map_;
// Compiled partition (fused kernel) cache
// Adopted from https://github.com/lamerman/cpp-lru-cache/blob/master/include/lrucache.hpp
...
thread_local std::unordered_map<std::vector<int64_t>, list_iterator_t>
    cache_items_map_;
thread_local int capacity_ = 75000;
...
void create_graph_sdpa_pattern_5(data_type dtype) {
  ...
  int patternID = dtype == data_type::bf16 ? ONEDNN_GRAPH_SDPA_PATTERN_5_BF16 : ONEDNN_GRAPH_SDPA_PATTERN_5_FP32;
  partition_map_[patternID] = std::move(partition);
}

// Execute SDPA partition
// TODO: creating a vector of input tensors wouldn't create extra copies of tensors.
// So, we can make this function generic.
// Also, we need to be able to handle multiple outputs anyway
...
// Compile SDPA pattern 5 corresponding to torch/_inductor/fx_passes/fuse_attention.py
// TODO: we could use this function generic by adding shape information in cp_entry
// before this function is called. Also, handle synchronization in case multiple
// thread-pools of the same PyTorch process are used for inference.  
void compile_and_cache_sdpa_pattern_5(
    ...
    cp_entry& cp) {
  // assuming all inputs have the same dtype. Might revisit this assumption later
  data_type dtype = query.scalar_type() == at::ScalarType::Float ? data_type::f32 : data_type::bf16;
  int i = 0;
  ...
  cp.inputLogicalTensors_.emplace_back(
      logical_tensor(i, dtype, attn_mask.sizes().vec(), attn_mask.strides().vec()));
  cp.inputLLGATensors_.emplace_back(RunArg(
      cp.inputLogicalTensors_[i++],
      onednn_graph::Engine::getEngine(),
      attn_mask.data_ptr()));
...
Tensor mkldnn_graph_sdpa_pattern(
  ...
  if (iter == cache_items_map_.end()) {
    ...
    auto retVal = execute_sdpa_partition(key, query, value, scale, attn_mask, compiledPartitionEntry);
